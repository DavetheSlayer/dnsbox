...
!    call io_loadState('state0000.h5')
    ...
    do n = 1, Nt
        ...
        do k=fstart(3),fend(3); do j=fstart(2),fend(2); do i=fstart(1),fend(1)
            ...
            uhat(i, j, k) = uhatold(i,j,k) + dt * rhsuhat(i, j, k) / 6.0d0
            uhattemp(i, j, k) = uhatold(i,j,k) + dt * rhsuhat(i, j, k) / 2.0d0
            
            vhat(i, j, k) = vhatold(i,j,k) + dt * rhsvhat(i, j, k) / 6.0d0
            vhattemp(i, j, k) = vhatold(i, j, k) + dt * rhsvhat(i, j, k) / 2.0d0
            
            what(i, j, k) = whatold(i,j,k) + dt * rhswhat(i, j, k) / 6.0d0
            whattemp(i, j, k) = whatold(i, j, k) + dt * rhswhat(i, j, k) / 2.0d0
        ...
        do k=fstart(3),fend(3); do j=fstart(2),fend(2); do i=fstart(1),fend(1)
            ...
            uhat(i, j, k) = uhat(i,j,k) + dt * rhsuhat(i, j, k) / 3.0d0
            uhattemp(i, j, k) = uhatold(i,j,k) + dt * rhsuhat(i, j, k) / 2.0d0
            
            vhat(i, j, k) = vhat(i,j,k) + dt * rhsvhat(i, j, k) / 3.0d0
            vhattemp(i, j, k) = vhatold(i, j, k) + dt * rhsvhat(i, j, k) / 2.0d0
            
            what(i, j, k) = what(i,j,k) + dt * rhswhat(i, j, k) / 3.0d0
            whattemp(i, j, k) = whatold(i, j, k) + dt * rhswhat(i, j, k) / 2.0d0
        ...
        do k=fstart(3),fend(3); do j=fstart(2),fend(2); do i=fstart(1),fend(1)
            ...
            uhat(i, j, k) = uhat(i,j,k) + dt * rhsuhat(i, j, k) / 3.0d0
            uhattemp(i, j, k) = uhatold(i, j, k) + dt * rhsuhat(i, j, k)
            
            vhat(i, j, k) = vhat(i,j,k) + dt * rhsvhat(i, j, k) / 3.0d0
            vhattemp(i, j, k) = vhatold(i, j, k) + dt * rhsvhat(i, j, k)
            
            what(i, j, k) = what(i,j,k) + dt * rhswhat(i, j, k) / 3.0d0
            whattemp(i, j, k) = whatold(i, j, k) + dt * rhswhat(i, j, k)
        end do; end do; end do
        ...
        do k=fstart(3),fend(3); do j=fstart(2),fend(2); do i=fstart(1),fend(1)
            ...
            uhat(i, j, k) = uhat(i,j,k) + dt * rhsuhat(i, j, k) / 6.0d0
            uhattemp(i, j, k) = uhat(i, j, k)
            uhatold(i, j, k) = uhat(i, j, k)
            
            
            vhat(i, j, k) = vhat(i,j,k) + dt * rhsvhat(i, j, k) / 6.0d0
            vhattemp(i, j, k) = vhat(i, j, k)
            vhatold(i, j, k) = vhat(i, j, k)
            
            what(i, j, k) = what(i,j,k) + dt * rhswhat(i, j, k) / 6.0d0
